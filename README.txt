...
SchevoGtk provides support for the PyGTK GUI toolkit through custom
widgets and a Navigator application that allows you to view and
interact with any Schevo database.  To accomplish this, we make use of
Glade files so that both the Navigator and custom applications can be
created in any designer that works with Glade files, such as the
Gazpacho GUI Designer.  We also make use of the Kiwi library and
framework, since Kiwi provides widgets that go beyond those provided
by PyGTK, and because the Kiwi widgets are designed for database
applications.
...
This toolkit is under active development.  If you would like to
...
they work well for Schevo.  That means that you will need to check out
the Subversion repositories for Gazpacho and Kiwi, and be prepared to
update them on a regular basis.  Do not install the packaged releases
of these applications as they are not current enough for our purposes.
...
You will also need to check out the SchevoGTK product.  The code can
...
http://schevo.org/svn/trunk/Schevo/

The setup program will add the SchevoGtk to your PYTHONPATH (via the
easy-install.pth file in Python24\Lib\site-packages) and will add a
"gnav" subcommand to the schevo commandline tool.  You should then be
...
C:\Code\Kiwi-svn\gazpacho-plugin;C:\Code\Schevo-svn\trunk\Gtk\schevogtk2\gazpacho-plugin
